...
import { traceToolExecution } from "../otel-instrumentation.js";
import {
  filterToolsByPolicy,
...
export async function handleToolsInvokeHttpRequest(
  ...
  try {
    ...
    const tracedExecute = traceToolExecution(
      toolName,
      (tool as any).execute?.bind(tool) ?? (async () => undefined)
    );
    const result = await tracedExecute(`http-${Date.now()}`, toolArgs);
